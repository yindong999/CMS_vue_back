<template>
  <div>
    ...
    <div class="center">
      <p
        style="height: 48px;border-bottom: 1px solid #E7E8E9;line-height: 48px;padding-left: 24px;color: #3C3D43;font-size: 16px;font-weight: 500;"
      >
        平台发布统计
        <span
          @click="handLook"
          style="display: inline-block;float: right;font-size: 14px;color: #767885;margin-right: 24px;cursor: pointer;"
        >
          详情
          <a-icon style="margin-left: 6px;" type="right" />
        </span>
      </p>
      ...
      <a-row :gutter="24" style="display:flex;align-items:center;margin-bottom: 25px;">
        ...
        <a-col :md="4" :sm="6">
          ...
          >
            ...
            <a-select-option
              :value="key"
              v-for="(value, key, index) in meidiaS"
              :key="index"
        ...
        <a-col :md="4" :sm="6">
          <a-date-picker
            placeholder="请选择开始日期"
            v-model="mytime_start"
            @change="onChangeA"
            style="width: 100%;"
          ></a-date-picker>
        </a-col>
        <a-col :md="4" :sm="6" style="margin-left: -16px;"> 
          <a-date-picker
            placeholder="请选择结束日期"
            v-model="mytime_end"
            @change="onChangeO"
...
export default {
  ...
  data() {
    return {
      ...
      startVal: 0,
      topData: [
        { id: 1, name: '账号数量', num: 0 },
        { id: 2, name: '推送发布数量', num: 0 },
        { id: 3, name: '报备数量', num: 0 }
      ],
      bottomData: [
        { id: 1, name: '转发量趋势', val: [] },
        { id: 2, name: '评论量趋势', val: [] },
  ...
  created() {
    //判断生产测试环境,变更公众号
    const evnUrl = baseUrl
    if (evnUrl.split('//')[1].split('-')[0] === 'haier') {
      this.codeImage = require('@/assets/gongzhonghao_prod.jpg')
    } else {
      this.codeImage = require('@/assets/gongzhonghao_test.jpg')
    }
	var userInfo = vue.ls.get(USER_INFO)
	// 在登录接口存下一个标志，防止每次进入首页都弹出二维码，只有登陆上第一次进入首页才弹出
    var isFirst = localStorage.getItem('isFirst')
    if (!userInfo.userWxOpenId && (userInfo.roleName.indexOf('后台审核人员') !== -1) && isFirst) {
      this.isShow = true
    }else{
      this.isShow = false
    }
  ...
  mounted() {
    ...
    trend('').then(res => {
      if (res.code === 200) {
        var rd = res.data
        //  res.data.contentTrend = res.data.contentTrend.map(item=>{
        //   console.log(item)
        //   item.微信公众号 = 0
        //   item.抖音 = 0
        //   item.新浪微博 = 0
        ...
        this.finsh = true
        console.log('res.data',res.data.contentTrend)
  ...
  methods: {
    stime2(e1, e2) {
      if (!e2) {
        ...
        this.queryParam.publishTimeS = e2
        this.st2 = e2
      }
    }, 
    etime2(e1, e2) {
      if (!e2) {
        ...
        this.queryParam.publishTimeE = e2
        this.et2 = e2
      }
    },
    ok3(e) {
      ...
      var startTime3 =
        ...
        ':' +
        time3.getSeconds()
      this.queryParam.publishTimeS = startTime3
    }, 
    ok4(e) {
      ...
      var endTime4 =
        ...
        ':' +
        time4.getSeconds()
      this.queryParam.publishTimeE = endTime4
    },
    disabledStartDate2(startValue) {
      const endValue = this.mytime_end
    ...
    disabledEndDate2(endValue) {
      const startValue = this.mytime_start
      ...
      if(e===''||e===undefined){
        ...
        var newStr2 = ''
         for(var i in this.meidiaS){
          newStr2 += this.meidiaS[i] + ','
         }
        newStr2 = newStr2.substr(0,newStr2.length-1)
        this.params.allMediaPlatform = newStr2
        ...
        this.params.mediaPlatform = this.meidiaS[e]
        this.params.allMediaPlatform = ''
        this.channelChangeC = ''
        this.getChannels()
      } 
    },
    ...
    getAll() {
      dataDictionary({ dataType: 'mediaPlatform' }).then(res => {
        if (res.code === 200) {
          // 目前是把媒体平台中的微信，微博，抖音的筛选出来了，只保留这3个
          var newMedia = {}
          for(var item in res.data){
            if(item === 'wechat' || item === 'sinaBlog' || item === 'douyin'){
              newMedia[item] = res.data[item]
            }
          }
          // this.meidiaS = res.data
          this.meidiaS = newMedia
          console.log('newMedia:newMedia',newMedia)
          var strs = ''
          for(var i in newMedia){
            strs += newMedia[i] + ','
          }
           strs = strs.substr(0,strs.length-1)
           this.params.allMediaPlatform = strs
           this.getChannels()
        } else if (res.code !== 400) {
          this.$message.error(res.message)
        }
    ...
    getChannels(){
      thirdPublishChannelList(this.params).then(res => {
        if (res.code === 200) {
      ...
      myChart.setOption({
        ...
        dataset: {
          source: [
            ...
            [
              red[0].trendDate,
              this.visitInfo[0].新浪微博 / 1,
              this.visitInfo[0].微信公众号 / 1,
              this.visitInfo[0].抖音 / 1
            ],
            [
              red[1].trendDate,
              this.visitInfo[1].新浪微博 / 1,
              this.visitInfo[1].微信公众号 / 1,
              this.visitInfo[1].抖音 / 1
            ],
            [
              red[2].trendDate,
              this.visitInfo[2].新浪微博 / 1,
              this.visitInfo[2].微信公众号 / 1,
              this.visitInfo[2].抖音 / 1
            ],
            [
              red[3].trendDate,
              this.visitInfo[3].新浪微博 / 1,
              this.visitInfo[3].微信公众号 / 1,
              this.visitInfo[3].抖音 / 1
            ],
            [
              red[4].trendDate,
              this.visitInfo[4].新浪微博 / 1,
              this.visitInfo[4].微信公众号 / 1,
              this.visitInfo[4].抖音 / 1
            ],
            [
              red[5].trendDate,
              this.visitInfo[5].新浪微博 / 1,
              this.visitInfo[5].微信公众号 / 1,
              this.visitInfo[5].抖音 / 1
            ],
            [
              red[6].trendDate,
              this.visitInfo[6].新浪微博 / 1,
              this.visitInfo[6].微信公众号 / 1,
              this.visitInfo[6].抖音 / 1
            ]
    ...
    handLook() {
      ...
      this.channelChangeC = ''
      this.bannlId = ''
      this.mytime_start = null
      this.mytime_end = null
      this.queryParam.publishTimeE = ''
      this.queryParam.publishTimeS = ''
      this.queryParam.mediaPlatform = ''
